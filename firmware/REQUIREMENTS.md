...
### 2.1 DRV8825 制御信号（4ch共通）

| 信号名 | 方向 | ESP32 GPIO | 説明 |
|--------|------|------------|------|
| STEP_0 | ESP32 → DRV8825 | GPIO4 | ステップパルス
| STEP_1 | ESP32 → DRV8825 | GPIO6 | ステップパルス
| STEP_2 | ESP32 → DRV8825 | GPIO8 | ステップパルス
| STEP_3 | ESP32 → DRV8825 | GPIO10 | ステップパルス
（立ち上がりエッジで1ステップ） |
| DIR_0 | ESP32 → DRV8825 | GPIO5 | 回転方向（H=正転 / L=逆転） |
| DIR_1 | ESP32 → DRV8825 | GPIO7 | 回転方向（H=正転 / L=逆転） |
| DIR_2 | ESP32 → DRV8825 | GPIO9 | 回転方向（H=正転 / L=逆転） |
| DIR_3 | ESP32 → DRV8825 | GPIO11 | 回転方向（H=正転 / L=逆転） |
...
- n = 0〜3（チャンネル番号）
- A相・B相の両エッジで計数（4逓倍モード）

### 2.3 ADCチャンネル（4ch）

| 信号 | 内容 |
|------|------|
| ADC0〜ADC3 | NJM2114 オペアンプ出力（各モーター軸の電流/電圧センシング） |

- 分解能：12bit（ESP32-S3 内蔵ADC）
- 入力レンジ：0〜3.3V（オペアンプゲイン設定に依存）
...
- 各軸に独立した RMT チャンネルを割り当てる（CH0〜CH3 = RMT CH0〜CH3）
...
- 起動時に M0/M1/M2（GPIO22/23/24）を NVS 設定値に応じて出力し、全軸共通の分割数を確定する
...
8. M0（GPIO22）/ M1（GPIO23）/ M2（GPIO24）を NVS 設定値に応じて出力する
...
6. `EVT FAULT <reason> <axis_mask>` をホストに送信する
   ...
   - axis_mask: 影響軸のビットフィールド（例: 全軸 = 0xF）
...
```
1. 参照軸 = |steps| が最大の軸（複数同値は軸番号の小さい方）
...
#### F-ADC-02: 電流換算
- ADC値をオペアンプゲインとシャント抵抗値から電流値（mA）に換算する
- ゲイン・シャント抵抗値は設定パラメータとしてNVSに保存する
...
| `SYNC_MOVE <n> <ax0> <st0> [<ax1> <st1>...]` | n:2〜4, ax:0〜3, st:±int32 | 多軸同期移動（F-MOT-11） | `OK`（完了時 `EVT SYNC_DONE <mask>`） |
...
### Phase 2：4軸制御・モーションプロファイル
- [ ] 4 軸独立 RMT チャンネル設定（F-MOT-01）
...
- [ ] PCNT によるエンコーダ 4 逓倍デコード（4 軸）（F-ENC-01）
...
- [ ] 総合テスト（4 軸同時動作、ホーミング、脱調検出、フォルト復帰、SYNC_MOVE 直線補間確認）
...
| 信号 | GPIO | 説明 |
|------|------|------|
| M0（MS0） | GPIO22 | マイクロステップ bit0 |
| M1（MS1） | GPIO23 | マイクロステップ bit1 |
| M2（MS2） | GPIO24 | マイクロステップ bit2 |

**機能への影響：**
- `SET MICROSTEP <div>` コマンドで実行時に動的変更可能（全軸停止後のみ）
- 設定値は NVS に保存され再起動後も維持される
- 起動シーケンスの Step 8 で GPIO22/23/24 を NVS 設定値に応じて出力する

**ハードウェア変更（回路図・基板への対応が必要）：**
1. schematic-plan.md の DIP スイッチ（3 ポジション）を削除する
2. MS0/MS1/MS2 ネットを ESP32-S3 の GPIO22/23/24 に直結する
...
**反映先：** F-MOT-02（Section 3.1）、起動シーケンス（F-MOT-03 Step 8）、Section 2.1 信号テーブル、CLAUDE.md GPIO マッピング
